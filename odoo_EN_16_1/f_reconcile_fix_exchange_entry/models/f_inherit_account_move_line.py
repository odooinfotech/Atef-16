...
class FAccountMoveLine(models.Model):
    _inherit = "account.move.line"


#falak = override reconcile to fix exchange entry 0
    @api.model
    def _prepare_reconciliation_single_partial(self, debit_vals, credit_vals):
        ...
        if debit_vals['currency'] == credit_vals['currency'] == company_currency \
           ...
           # falak - comment this original  line  :  debit_rate = get_odoo_rate(debit_vals, other_line=credit_vals)
            # start -- falak
            debit_rate = get_accounting_rate(credit_vals)
            # finish -- falak
            ...
            # comment by falak this is original line : credit_rate = get_odoo_rate(credit_vals, other_line=debit_vals)
            # falak - new line start
            credit_rate = get_accounting_rate(debit_vals)
            # falak -- end
